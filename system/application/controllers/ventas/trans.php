        ...
        if ($u->save()) {
           ...
           echo form_hidden("cproducto_id$line", "$u->id");
            echo "<img src=\"" . base_url() . "images/ok.png\" width=\"20px\" title=\"Guardado\"/>
             <input type='hidden' value='$afect_global' name='cambio_global$line' id='cambio_global$line'>";
        } else
            show_error("" . $u->error->string);
    }
...
//        $promo->cproductos_subfamilias = $subfamilias_str;
        ...
        $promo->cproductos_familias=$this->input->post('cfamilia_id');
        $promo->cproductos_subfamilias=$this->input->post('csubfamilia_id');
        if ($promo->save()) {
