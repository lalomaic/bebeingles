...
class Grupo_c extends Controller {
	...
	function formulario() {
		...
		if (is_array($data['colect1']) and $usuarioid > 0 and $data['permisos'] != false and $this->session->userdata('logged_in') == TRUE) {
			$main_view = true;
			if ($subfuncion == "alta_grupo") {
				//Cargar los datos para el formulario
				//Definir la vista
				$data['principal'] = $ruta . "/" . $subfuncion;
				//Obtener Datos
				$data['validation'] = $this->admin_validacion->validacion_grupo();
				$data['modulos'] = $this->modulo->get_modulos();
				$data['asm'] = $this->accion->get_asm();
				if ($data['asm'] == false)
					show_error("No hay catálogo de acciones registradas en el sistema, contacte al Administrador del Sistema");
			} else if ($subfuncion == "list_grupos") {

				if ($this->uri->segment(5) == "editar_grupo") {
					//Cargar los datos para el formulario
					$edit_usr = $this->uri->segment(5);
					$id = $this->uri->segment(6);
					//Definir la vista
					$data['principal'] = $ruta . "/editar_grupo";
					//Obtener Datos
					$data['validation'] = $this->admin_validacion->validacion_grupo();
					$data['grupo'] = $this->grupo->get_grupo($id);
				} else if ($this->uri->segment(5) == "asignar_permisos_grupo") {
					...
					//Obtener Datos
					$data['grupo'] = $this->grupo->get_grupo($id);
					$data['title'] = "Asignar Permisos al Grupo: <strong>" . $data['grupo']->nombre . "</strong>";
					$data['modulos'] = $this->modulo->get_modulos();
					//Matriz asociativa PHP con accion_id ->key y permiso as $value
					$data['grupo_permisos'] = $this->grupo_accion->get_grupo_accion($id);
					//Obtener el Catálogo de acciones, submodulos y modulos globales (ASM)
					$data['asm'] = $this->accion->get_asm($id);
					if ($data['asm'] == false)
						show_error("No hay catálogo de acciones registradas en el sistema, contacte al Administrador del Sistema");
				} else if ($this->uri->segment(5) == "borrar_grupo") {
					$id = $this->uri->segment(6);
					if (substr(decbin($permisos), 2, 1) == 1) {
						...
						$data['error_field'] = "No tiene permisos para deshabilitar el Grupo";
					}
				} else {
					...
					$config['base_url'] = base_url() . "index.php/" . $ruta . "/" . $controller . "/" . $funcion . "/list_grupos/";
					$config['per_page'] = '15';
					$page = $this->uri->segment(5);
					...
					$data['grupos'] = $this->grupo->get_grupos_list($offset, $config['per_page']);
