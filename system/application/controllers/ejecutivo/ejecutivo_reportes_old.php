...
class Ejecutivo_reportes extends Controller {
	...
	function rep_graficas_ventas_ub() {  // BEGIN method rep_productos_pdf
		...
		foreach($global->result() as $row){
			if($row->tag!=''){
				...
				//Pares vendidos
				$datos=$this->db->query("select sum(s.cantidad) as pares from salidas as s $where_s  and s.espacios_fisicos_id='$row->espacio_id' and s.estatus_general_id=1 and ctipo_salida_id=1");
				if($datos->num_rows()==1)
					$pares=$datos->row();
				else
					$pares->pares=0;
				//Devoluciones
				$dev=$this->db->query("select sum(d.cantidad) as devoluciones from devoluciones as d where fecha_devolucion>='$fecha1' and fecha_devolucion<='$fecha2' and d.espacio_fisico_id='$row->espacio_id' and estatus_general_id=1 ");
				if($dev->num_rows()==1){
					$pares_dev=$dev->row();
				...
				if($datos->num_rows()==1)
					...
					$t->monto=0;
				//Fiscal
				$datos1=$this->db->query("select sum(c.venta_fiscal) as monto from cortes_diarios as c  $where_c and c.espacios_fisicos_id='$row->espacio_id'");
				...
				//Otros Egresos
				$datos=$this->db->query("select sum(oe.monto) as monto from otros_egresos_detalles as oe left join cotros_egresos as co on co.id=oe.cotros_egresos_id $where_o and oe.espacios_fisicos_id='$row->espacio_id' and oe.estatus_general_id=1");
				if($datos->num_rows()==1)
					$otros=$datos->row();
				else
					$otros->monto=0;
	...
	function rep_hora_ventas() {
		...
		if((int)$_POST['espaciof']>0){
			$filtro[] = "nota_remision.espacios_fisicos_id = ".(int)$_POST['espaciof'];
		...
		from nota_remision
		left join espacios_fisicos as e on e.id=nota_remision.espacios_fisicos_id
	...
	function rep_top_productos_pdf() { // BEGIN method kardex_pdf
		...
		if ($espacio != '0') {
			...
			$where2 .= " AND n.espacios_fisicos_id = '$espacio'";
		}
		...
		and s.espacios_fisicos_id = n.espacios_fisicos_id
	...
	function rep_rentabilidad_pdf() { // BEGIN method rep_productos_pdf
		...
		foreach($espacios->all as $row){
			...
			$datos=$this->db->query("select sum(g.monto) as monto from gastos_detalles as g left join cgastos as cg on cg.id=g.cgastos_id $where_g and g.espacios_fisicos_id='$row->id' and g.estatus_general_id=1 ");
			if($datos->num_rows()==1)
				$t=$datos->row();
			else
				$t->monto=0;
			...
			//Fiscal
			$datos1=$this->db->query("select sum(c.venta_fiscal) as monto from cortes_diarios as c  $where_c and c.espacios_fisicos_id='$row->id'");
	...
	function rep_comparativo_marcas_pdf(){
		...
		//Obtener el query por espacio fisico y asignarlo a una matriz de PHP
		$matriz_p=array();
		$datos=$this->db->query("select distinct(e.cproductos_id) as cproductos_id, p.descripcion  from entradas as e left join cproductos as p on p.id=e.cproductos_id where lote_id in ($lotes_str) and e.espacios_fisicos_id in ($almacenes_str) and e.estatus_general_id='1' and cmarca_producto_id='$marca_id' order by descripcion ");
