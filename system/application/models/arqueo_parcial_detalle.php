 ...
 * @link
 */
class Arqueo_parcial_detalle extends DataMapper {
	var $table= "arqueo_parcial_detalles";


	function  Arqueo_parcial_detalle($id=null)
	{
	...
	{
		// Create a temporary user object
		$u = new Arqueo_parcial_detalle();
		//Buscar en la base de datos
		$sql="select ad.id as llave, ad.*, u.nombre as usuario, ef.tag as espacio, p.descripcion as producto,  pn.numero_mm as numero, ta.tag as accion, ta.id as accion_id from arqueo_parcial_detalles as ad left join usuarios as u on u.id=ad.usuario_id left join arqueos_parciales as a on a.id=ad.arqueo_parcial_id left join ctipo_ajuste_detalles as ta on ta.id=ad.ctipo_ajuste_detalle_id left join espacios_fisicos as ef on ef.id=a.espacio_fisico_id left join cproductos_numeracion as pn on pn.id=ad.cproductos_numero_id left join cproductos as p on p.id=ad.cproducto_id where arqueo_parcial_id='$id'  and ad.estatus_general_id=1 order by producto ";
		//Buscar en la base de datos
		$u->query($sql);
		if($u->c_rows >0){
			return $u;
