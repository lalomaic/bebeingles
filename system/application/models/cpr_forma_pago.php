 ...
 * @link
 */
class Cpr_forma_pago extends DataMapper {

	var $table= "cpr_formas_pago";


	function  Cpr_forma_pago($id=null)
	{
	...
	function get_cpr_forma_pago($id)
	{
        ...
        function get_formas_pago_pdf()
	{
            $fp = new Cpr_forma_pago();            
            $sql="select 
                p.id,p.tag,e.tag as estatus 
                from cpr_formas_pago as p 
                left join 
                estatus_general as e 
                on e.id=p.estatus_general_id";
	    $fp->query($sql);
            if($fp->c_rows > 0)
		return $fp;
            else 
