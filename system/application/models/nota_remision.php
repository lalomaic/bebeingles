...
class Nota_remision extends DataMapper {
	...
	function get_ventas_gral_empleados($fecha1,$fecha2,$espacio){
		//$espacio=0;
		if($espacio>0){
		...
		$u = new Salida();
		$sql="select distinct(n.numero_remision_interno) as rem,n.importe_total, cobro_vales, sum(s.costo_total),sum(cantidad) as cantidad,sum(cp.precio_compra) as precio_compra, 
		n.empleado_id,es.tag,s.espacios_fisicos_id as espacio_sa,n.espacios_fisicos_id as espacio_nota,(e.nombre || ' ' || e.apaterno || ' ' || e.amaterno) as empleado,
		comi.criterio1 from salidas as s 
		left join nota_remision as n on n.numero_remision_interno=s.numero_remision_id and n.espacios_fisicos_id=s.espacios_fisicos_id 
		left join empleados as e on e.id=n.empleado_id 
		left join comisiones as comi on comi.id=e.comision_id
                left join espacios_fisicos as es on es.id=n.espacios_fisicos_id
                left join cproductos as cp on cp.id=s.cproductos_id
		where n.estatus_general_id='1'
		and s.estatus_general_id='1' and n.nota_apartado_id='0'
		and ctipo_salida_id='1' $where 
		group by n.numero_remision_interno,es.tag,comi.criterio1, n.importe_total,cobro_vales,n.empleado_id, s.espacios_fisicos_id,n.espacios_fisicos_id, e.nombre, e.apaterno, e.amaterno order by empleado,espacio_nota asc ";
	...
	function get_ventas_empleados($fecha1,$fecha2,$espacio){
		...
		and s.estatus_general_id='1' and n.nota_apartado_id='0'
		and ctipo_salida_id='1' $where 
		group by n.numero_remision_interno,es.tag,comi.criterio1, n.importe_total,cobro_vales,n.empleado_id, s.espacios_fisicos_id,n.espacios_fisicos_id, e.nombre, e.apaterno, e.amaterno order by espacio_nota,empleado asc ";
...
function get_ventas_empleados_flete($fecha1,$fecha2,$espacio){
		...
		$u = new Salida();
		$sql="select distinct(n.numero_remision_interno) as rem,n.importe_total, cobro_vales, sum(s.costo_total),sum(cantidad) as cantidad, n.empleado_id,s.espacios_fisicos_id,n.espacios_fisicos_id as espacio_no from salidas as s 
left join nota_remision as n on n.numero_remision_interno=s.numero_remision_id and n.espacios_fisicos_id=s.espacios_fisicos_id
left join empleados as e on e.id=n.empleado_id 
		where n.estatus_general_id='1'
		and s.estatus_general_id='1' and s.cproductos_id=3524
		and ctipo_salida_id='1' $where  
group by n.numero_remision_interno, n.importe_total,cobro_vales,n.empleado_id, s.espacios_fisicos_id,n.espacios_fisicos_id, e.nombre, e.apaterno, e.amaterno order by espacio_no";
	...
	function get_abonos_empleados_vales($fecha1,$fecha2,$espacio){
		...
		$u = new Salida();
		$sql="select sum(ab.efectivo)+sum(ab.tarjeta)+sum(ab.cobro_cheques)+sum(ab.cobro_transferencia) as monto,empleado_id,
		na.espacio_fisico_id as espacio_vales,(e.nombre || ' ' || e.apaterno || ' ' || e.amaterno) as empleado 
		from abonos_pv as ab 
left join nota_apartado as na on na.id_ubicacion_local=ab.nota_apartado_id and na.espacio_fisico_id=ab.espacio_fisico_id
left join empleados as e on e.id=na.empleado_id 
where ab.vale_apartado>0 $where and ab.estatus_general_id=2
group by na.empleado_id,na.espacio_fisico_id,e.nombre,e.apaterno,e.amaterno order by espacio_vales,empleado asc";
		...
		function get_abonos_empleados_cancelados($fecha1,$fecha2,$espacio){
			//$espacio=0;
			if($espacio>0){
				$where= " and ab.espacio_fisico_id='$espacio' and na.espacio_fisico_id='$espacio' ";
			} else
	...
	left join nota_apartado as na on na.id_ubicacion_local=ab.nota_apartado_id and na.espacio_fisico_id=ab.espacio_fisico_id
	left join empleados as e on e.id=na.empleado_id 
	where ab.vale_apartado=0 and ab.estatus_general_id=2 $where 
	group by na.empleado_id,na.espacio_fisico_id,e.nombre,e.apaterno,e.amaterno order by espacio_vales,empleado asc";
   ...
   function get_abonos_empleados($fecha1,$fecha2,$espacio){
		...
		$u = new Salida();
		$sql="select sum(ab.efectivo)+sum(ab.tarjeta)+sum(ab.cobro_cheques)+sum(ab.cobro_transferencia) as monto,empleado_id,
		na.espacio_fisico_id as espacio_abonos,(e.nombre || ' ' || e.apaterno || ' ' || e.amaterno) as empleado
		 from abonos_pv as ab 
left join nota_apartado as na on na.id_ubicacion_local=ab.nota_apartado_id and na.espacio_fisico_id=ab.espacio_fisico_id
left join empleados as e on e.id=na.empleado_id 
where na.estatus_general_id=1 $where  and ab.estatus_general_id=1
group by na.empleado_id,na.espacio_fisico_id,e.nombre,e.apaterno,e.amaterno order by na.espacio_fisico_id,empleado asc";
...
return $this;
    ...
    $sql="select distinct(n.espacios_fisicos_id) as espacio_id, 
        ...
        where n.estatus_general_id='1' and n.nota_apartado_id=0
 ...
 function get_empleados_total(){
        
    $u = new Empleado();
		$sql="select id,(nombre || ' ' || apaterno || ' ' || amaterno) as empleado from empleados order by empleado ";
		//Buscar en la base de datos
		$u->query($sql);
		if($u->c_rows > 0){
			return $u;
