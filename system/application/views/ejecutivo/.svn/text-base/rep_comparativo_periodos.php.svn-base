...
function send1(){
	if($('#fecha1').val() =='' || $('#fecha2').val()=='' || $('#fecha3').val()=='' || $('#fecha4').val()==''){
		alert("Seleccione las cuatro fechas para generar el reporte");
	} else {
		$('#pdf').show();
...
echo form_open($ruta."/".$ruta."_reportes/rep_comparativo_periodos_pdf/", $atrib) . "\n";
...
<table width="100%" class='form_table'>
	<tr>
		...
		<th valign="top">Periodo No. 1</th>
		<th valign="top">Periodo No. 2</th>
	</tr>
	<tr>
		<td align="center">
			<table>
		...
		<td>Fecha Inicio 1<?=form_input($fecha1)?><br /> Fecha Final 1<?=form_input($fecha2)?><br />
		</td>
		<td>Fecha Inicio 2<?=form_input($fecha3)?><br /> Fecha Final 2<?=form_input($fecha4)?><br />
		</td>
	</tr>
	<tr>
