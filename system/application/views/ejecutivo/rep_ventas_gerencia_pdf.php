...
$pdf->Cell(0,5,"REPORTE DE VENTAS POR SURCURSAL",0,1,'L');
...
if($ventas_empleados!=false){
	...
	$pdf->SetWidths(array(5,35,18,25,25,18,22,25,25));
	$pdf->Row(array("#","Tienda","Cantidad Productos", "Ventas","APartados","Gastos","Flete","Costo","Utilidad"));
	$t_importe=0; $t_pares=0; $espacio=0;$colect=array();$nota_re=0;$comision_t=0;$flete_t=0; $t_apartado=0;$t_gasto=0; $total_gasto=0;$utilidad=0;$t_flete=0;$costo_total=0;
	foreach($ventas_empleados->all as $row) {
		 ...
		 if($espacio!=$row->espacio_nota){
			...
			$colect[$g]["importe_neto"]=0;
                   ...
                   $colect[$g]["total_precio"]+=$row->precio_compra;
...
$pdf->Row(array($y,$row1['espacio_tag'],number_format($row1['pares'], 0, ".",","),$importe_t,number_format($monto+$bono_vales,2,".",","), number_format($gasto_espacio,2,".",",") ,"$".number_format($flete,2,".",","),  number_format($row1['total_precio'],2,".",","),  number_format($row1['importe_neto']+$monto+$bono_vales-$flete-$gasto_espacio-$row1['total_precio'],2,".",",")));
		//$t_importe+=$row1['importe_neto'];
                $t_importe+=$row1['importe_neto']+$monto+$bono_vales-$flete;
                $utilidad+=$row1['importe_neto']+$monto+$bono_vales-$flete-$gasto_espacio-$row1['total_precio'];
		$t_pares+=$row1['pares'];
   $comision_t+=$t_apartado;
   $t_flete+=$flete_t;
   $total_gasto=$t_gasto;
   $costo_total+=$row1['total_precio'];
	$y+1;	
                
	}
	$pdf->SetFont('Times','B',8);
     $pdf->Row(array('','TOTAL',  number_format($t_pares,2,".",","),number_format($t_importe, 2, ".",","),number_format($comision_t, 2, ".",","),number_format($total_gasto, 2, ".",","),number_format($t_flete, 2, ".",","),  number_format($costo_total,2,".",","),number_format($utilidad, 2, ".",",")));
