...
function fil_proveedor(){
	...
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras/"+pag+"/proveedor/"+ped;
}
function fil_marca(){
	pag=<? echo $pag;?>;
	ped=$("#cmarca_id").val();
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras/"+pag+"/marcas/"+ped;
}
function fil_pedido_id(){
	pag=<? echo $pag;?>;
	ped=$("#pedido_id").val();
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras/"+pag+"/pedido_id/"+ped;
}
...
<table class="listado" border="0" width="90%">
	...
	<script>
function auth(id){
	$("#s"+id).html('Enviando Correo al proveedor...');
	alert("Se enviará por correo electrónico el pedido al proveedor, Aceptar para continuar");
	key=$('#passwd'+id).val();
	if(key.length>0 && key!= ''){
		llave=hex_md5(key);
	...
	foreach($pr_pedidos->all as $row) {
		...
		$edit="<a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/editar_pre_pedido/".$row->id."\"><img src=\"".base_url()."images/edit.png\" width=\"25px\" title=\"Editar Pedido\" border=\"0\"></a>";


		if($row->cpr_estatus_pedido_id == 5){
			$auth="<a href=\"javascript:pre_auth($row->id)\" >$validar</a>";
			$pass="<div id='$row->id' class='aut'><input type='password' value='' id='passwd$row->id' size='8'><a href='javascript:auth($row->id)'><span id='s$row->id'>Autorizar</span></a></div>";
			$clonar="<a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/clonar_pedido/".$row->id."\" target='_blank' class='modal_upload'><img src=\"".base_url()."images/clonar.png\" width=\"25px\" title=\"Clonar el  Pedido $row->id\" border=\"0\"></a>";
		}

		if($row->estatus=="Cancelado"){
			$delete=''; $edit=''; $auth=''; $photo=''; $codbar=''; $clonar='';
		}
		...
		echo "<tr background=\"$img_row\"><td align=\"center\">$row->id</td><td align=\"center\">".fecha_imp($row->fecha_alta)."</td><td align=\"center\">".fecha_imp($row->fecha_entrega)."</td><td>$row->proveedor</td><td>$row->marca</td><td>$row->espacio_fisico</td><td align=\"right\">$ ".number_format($row->monto_total, 2, ".", ",")."</td><td align=\"center\">$row->estatus</td><td align=\"center\">$row->usuario</td><td>$edit $delete<a href=\"".base_url()."index.php/".$ruta."/compras_reportes/rep_pedido_compra/".$row->id."\" target=\"_blank\">$photo</a>$auth $pass <a href=\"".base_url()."index.php/compras/compras_reportes/rep_etiquetas_codigo_barras_pdf/".$row->id."\" target='_blank'>$codbar</a> $clonar</td></tr>";
