...
$url_form=base_url()."index.php/tienda/trans/editar_salida_traspasos";
$this->load->view('tienda/js_editar_transferencia');
...
echo "<tr><td class='form_tag'><label for=\"ubicacion_salida_id\">Almac�n/Tienda solicitante: </label>"; echo form_dropdown('ubicacion_salida_id', $select2, "$traspaso->ubicacion_entrada_id", "disabled='disabled' id='ubicacion_salida'");echo "</td><td class='form_input'><label for=\"cpr_estatus_pedido_id\">Estatus del Pedido: </label>"; echo form_dropdown('cpr_estatus_pedido_id', $select4, "2", "disabled='disabled'"); echo "</td></tr>";
	...
	<table width="870" class="row_detail_pred" id="header">
		...
		<table width="700" class="row_detail_prev" id="total">
			<tr>
				...
				echo "<button type='button' onclick=\"window.location='".base_url()."index.php/tienda/tienda_c/formulario/list_salida_traspasos/'\">Cerrar</button>";
				if(substr(decbin($permisos), 0, 1)==1){
