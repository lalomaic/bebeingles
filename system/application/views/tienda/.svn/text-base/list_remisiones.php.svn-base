...
<table class="listado" border="0" width="700">
	...
	if($remisiones==false)
		...
		foreach($remisiones->all as $row) {
			...
			echo "<tr background=\"$img_row\"><td align=\"center\">$row->id</td><td align=\"center\">$row->tag</td><td align='center'>$row->numero_remision</td><td align=\"center\">". fecha_imp($row->fecha, 1)."</td><td align=\"right\">$row->hora</td><td align=\"right\">$ ".number_format($row->importe_total, 2, ".", ",")."</td><td align='center'>$estatus</td><td></td></tr> \n";
