<?php
foreach($plantilla->all as $lineap){
	?>
<script>
...
echo form_open($ruta.'/trans/editar_detalles_stock', $atrib) . "\n";
...
for($x=0;$x< count($productos)-2; $x=$x+3){
	...
	echo "<td class=\"stock_detalle\" $style>"; echo form_checkbox("producto$x", $productos[$x]['id'], $check, "class=\"chk\" id=\"chk$x\"");echo $productos[$x]['descripcion']."<br/><div style='text-align:right;'>"; echo form_hidden("id$x", $productos[$x]['d_id']); echo form_input("cantidad$x", $productos[$x]['cantidad'], 'size="12" class="subtotal" id="cantidad'.$x.'"'); echo "</div></td>";
	$fid_prev=$productos[$x]['cfamilia_id'];

	if($productos[$x+1]['cantidad']==0){
		$check1=FALSE;
		$style1="style=\"background-color:yellow;\"";
	} else {
		$check1=TRUE;
		$style1="";
	...
	if($productos[$x]['cfamilia_id']==$productos[$x+1]['cfamilia_id']){
		echo "<td class='stock_detalle' $style1>"; echo form_checkbox("producto".($x+1), $productos[$x+1]['id'], $check1, "class=\"chk\" id=\"chk".($x+1)."\""); echo $productos[$x+1]['descripcion'] ."<br/><div style='text-align:right;'>"; echo form_hidden("id".($x+1), $productos[$x+1]['d_id']); echo form_input("cantidad".($x+1), $productos[$x+1]['cantidad'], 'size="12" class="subtotal" id="cantidad'.($x+1).'"'); echo "</div></td>";
	} else {
		echo "</tr>";
	...
	if($productos[$x+1]['cfamilia_id']==$productos[$x+2]['cfamilia_id']){
		echo "<td class=\"stock_detalle\" $style2>"; echo form_checkbox("producto".($x+2), $productos[$x+2]['id'], $check2, "class=\"chk\" id=\"chk".($x+2)."\"");echo $productos[$x+2]['descripcion']."<br/><div style='text-align:right;'>"; echo form_hidden("id".($x+2), $productos[$x+2]['d_id']); echo form_input("cantidad".($x+2), $productos[$x+2]['cantidad'], 'size="12" class="subtotal" id="cantidad'.($x+2).'"'); echo "</div></td>";
	} else {
		echo "</tr>";
