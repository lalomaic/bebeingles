<!DOCTYPE HTML>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Subir Inventario</title>
<link rel="stylesheet"
	href="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.9/themes/base/jquery-ui.css"
	id="theme">
<link rel="stylesheet"
	href="<? echo base_url();?>css/jquery.fileupload-ui.css">
<style>
body {
	font-family: Verdana, Arial, sans-serif;
	font-size: 13px;
	margin: 0;
	padding: 20px;
}
</style>
</head>
<body style="width: 100%; text-align: center;">
	<div id="foto" style="width: 100%; text-align: center;">
		<?php
		if(strlen($arqueo->ruta_archivo)>0){
			echo  "<a href=\"".base_url()."$arqueo->ruta_archivo\" target=\"_blank\">$arqueo->ruta_archivo</a><br/><br/>";
		}

		?>
	</div>
	<h3>
		<?php echo "Número de Inventario: ". $arqueo->id;?>
	</h3>
	<form id="file_upload"
		action="<? echo base_url();?>index.php/ajax_pet/subir_inventario_parcial/<?=$arqueo->id?>"
		method="POST" enctype="multipart/form-data">
		<input type="file" name="file" multiple>
		<button>Upload</button>
		<div>Subir Archivo de Inventario CSV</div>
	</form>
	<table id="files">

	</table>
	<script
		src="http://ajax.googleapis.com/ajax/libs/jquery/1.5.0/jquery.min.js"></script>
	<script
		src="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.9/jquery-ui.min.js"></script>
	<script src="<? echo base_url();?>css/jquery.fileupload.js"></script>
	<script src="<? echo base_url();?>css/jquery.fileupload-ui.js"></script>
	<script>
/*global $ */
$(function () {
    $('#file_upload').fileUploadUI({
        uploadTable: $('#files'),
        downloadTable: $('#files'),
        buildUploadRow: function (files, index) {
            return $('<tr><td>' + files[index].name + '<\/td>' +
                    '<td class="file_upload_progress"><div><\/div><\/td>' +
                    '<td class="file_upload_cancel">' +
                    '<button class="ui-state-default ui-corner-all" title="Cancelar">' +
                    '<span class="ui-icon ui-icon-cancel">Cancelar<\/span>' +
                    '<\/button><\/td><\/tr>');
        },
        buildDownloadRow: function (file) {
			$('#foto').html('');
			$('#foto').html('<tr><td align="center"><a href="<?=base_url()?>inventarios/'+file.rutafoto+'" target="_blank">'+file.rutafoto+'</a></td></tr>');
        }
    });
});
</script>
</body>
</html>
