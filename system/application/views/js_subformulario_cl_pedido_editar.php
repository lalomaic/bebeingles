...
function sendValue(valor, line){
    // post(file, data, callback, type); (only "file" is required)
    if(valor>0){
    $.post("<? echo base_url();?>index.php/ajax_pet/precios",{ enviarvalor: valor, linea: line},  // create an object will all values
    //function that is called when server returns a value.
    function(data){
        $('#precio'+line).html(data);
	calc(line);
    }
    );
   } else {
        $('#precio'+line).html("<input type='input' name='precio_u"+line+"' id='precio_u"+line+"' value='0' readonly='readonly' size='4'>");
	calc(line);
  }
}
...
function borrar_detalle(id, line){
   ...
   function calc(line){
    ...
    if(_IsNumber(unit) && producto>0 && precio>0 && unit.length>0 && imp.length>0){
      ...
      elem = eval("document.getElementById('table_row"+ next +"')");
      elem.style.display = 'block';
      var colect=0;
      tot.value=0;
	 ...
	 if(sub.length>0){
      ...
      pedidof=eval(path1+".cl_pedidos_id"+line);
