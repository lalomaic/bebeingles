...
function fil_sucursal(){
	...
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras_aut/"+pag+"/sucursal/"+ped;
}
function fil_proveedor(){
	pag=<? echo $pag;?>;
	ped=$("#cproveedor_id").val();
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras_aut/"+pag+"/proveedor/"+ped;
}
function fil_marca(){
	pag=<? echo $pag;?>;
	ped=$("#cmarca_id").val();
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras_aut/"+pag+"/marcas/"+ped;
}
function fil_pedido_id(){
	pag=<? echo $pag;?>;
	ped=$("#pedido_id").val();
	location.href="<?php echo base_url(); ?>index.php/compras/compras_c/formulario/list_compras_aut/"+pag+"/pedido_id/"+ped;
}
 ...
 function auth(id){
  ...
  if(key.length>0 && key!= ''){
	...
	foreach($pr_pedidos->all as $row) {
		...
		$edit="<a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/editar_pedido_compra/".$row->id."\" target='_blank'><img src=\"".base_url()."images/edit.png\" width=\"25px\" title=\"Editar Pedido\" border=\"0\"></a>";
		...
		echo "<tr background=\"$img_row\"><td align=\"center\">$row->id</td><td align=\"center\">".fecha_imp($row->fecha_alta)."</td><td align=\"center\">".fecha_imp($row->fecha_entrega)."</td><td>$row->proveedor</td><td>$row->marca</td><td>$row->espacio_fisico</td><td align=\"right\">$ ".number_format($row->monto_total, 2, ".", ",")."</td><td align=\"center\">$tipo</td><td align=\"center\">$row->estatus</td><td align=\"center\">$row->usuario</td><td>$edit $delete<a href=\"".base_url()."index.php/".$ruta."/compras_reportes/rep_pedido_compra/".$row->id."\" target=\"_blank\">$photo</a>$auth $pass <a href=\"".base_url()."index.php/".$ruta."/compras_reportes/rep_etiquetas_codigo_barras_pdf/".$row->id."\" target='_blank'>$codbar</a></td></tr>";
