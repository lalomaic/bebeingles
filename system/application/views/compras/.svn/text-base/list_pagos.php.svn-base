<?php 
echo "<h2>$title</h2>";
echo "<div align=\"left\"><a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/alta_pago\"><img src=\"".base_url()."images/adduser.png\" width=\"50px\" title=\"Nuevo Pago\"></a></div>";
...
<table class="listado" border="0">
	...
	foreach($pagos->all as $row) {
		...
		echo "<tr background=\"$img_row\"><td>$row->id</td><td>$row->factura</td><td>$row->forma_pago</td><td>$row->tipo_pago</td><td>$row->numero_cuenta</td><td>$row->monto_pagado</td><td>$row->fecha</td><td><a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/editar_pago/".$row->id."\">$edit</a>$delete<a href=\"".base_url()."index.php/welcome/sespolice/quejas_pdf/".$row->id."\" target=\"_blank\">$photo</a></td></tr>";
