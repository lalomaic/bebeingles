...
<?php
for($x=0;$x<$lineas;$x++){
	$fecha['id'].=$x; $concepto['id'].=$x; $monto['id'].=$x;

	echo "<tr><td>"; echo form_input($fecha); echo "</td>"; 
	echo "<td>"; 
	echo form_dropdown("cgasto_id$x", $catalogo_gastos, 0,"id='cgasto_id$x'"); 
	echo "</td><td>"; echo form_dropdown("cuenta_empresa$x", $select3, 0, "id='cuenta_empresa$x'"); 
	echo "</td><td>"; echo form_input($referencia); echo"</td><td>"; 
	echo form_input($concepto); echo "</td><td align='right'><span id='estatus$x'></span>"; 
	echo form_input($monto); echo "</td></tr>";

	$fecha['id']="fecha";	$concepto['id']="concepto"; $monto['id']="monto";
