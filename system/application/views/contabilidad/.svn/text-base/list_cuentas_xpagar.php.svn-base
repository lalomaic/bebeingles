...
<table class="listado" border="0" width="90%">
	...
	if($pr_facturas!=false){
		foreach($pr_facturas->all as $row) {
			...
			echo "<tr background=\"$img_row\"><td align=\"center\" class=\"detalle$alert\">$row->id</td><td align=\"center\">".fecha_imp($row->fecha)."</td><td align=\"center\">$fecha_pago</td><td>$row->proveedor</td><td>$empresa</td><td align=\"center\">$row->folio_factura</td><td align=\"right\">$ ".number_format($row->monto_total, 2, ".", ",")."</td><td>$row->usuario</td><td><a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/editar_pr_factura/".$row->id."\">$edit</a><a href=\"".base_url()."index.php/compras/compras_reportes/rep_pr_factura_pdf/".$row->id."\" class=\"modal_pdf\">$photo</a></td></tr>";
