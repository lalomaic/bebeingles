<?php
$ruta_auth=base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/borrar_compra/";
?>
<script>
...
function fil_sucursal(){
	...
	location.href="<?php echo base_url(); ?>index.php/contabilidad/contabilidad_c/formulario/list_general_compras_validadas/"+pag+"/sucursal/"+ped;
}
function fil_proveedor(){
	pag=<? echo $pag;?>;
	ped=$("#cproveedor_id").val();
	location.href="<?php echo base_url(); ?>index.php/contabilidad/contabilidad_c/formulario/list_general_compras_validadas/"+pag+"/proveedor/"+ped;
}
function fil_marca(){
	pag=<? echo $pag;?>;
	ped=$("#cmarca_id").val();
	location.href="<?php echo base_url(); ?>index.php/contabilidad/contabilidad_c/formulario/list_general_compras_validadas/"+pag+"/marcas/"+ped;
}

function fil_factura_id(){
	pag=<? echo $pag;?>;
	ped=$("#factura_id").val();
	location.href="<?php echo base_url(); ?>index.php/contabilidad/contabilidad_c/formulario/list_general_compras_validadas/"+pag+"/factura_id/"+ped;
}
...
echo "<a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/list_general_compras\" title=\"Listado de Entradas Sin validadas\"><img src=\"".base_url()."images/factura.png\" width=\"25px\"  > Compras Sin validadar </a></div>";
...
<table class="listado" border="0" width="900">
	...
	$trash="<img src=\"".base_url()."images/trash.png\" width=\"20px\" title=\"Deshabilitar Entrada\" border=\"0\">";
	$delete="";
	$edit="";
	if($entradas!=false){
		foreach($entradas->all as $row) {
			$row->estatus="Validada";
			$auth="<a href=\"javascript:pre_auth($row->pr_facturas_id)\" >$trash</a>";
			$pass="<div id='$row->pr_facturas_id' class='aut'><input type='password' value='' id='passwd$row->pr_facturas_id' size='8'><a href='javascript:auth($row->pr_facturas_id)'>Eliminar</a></div>";

			//Vencimiento
			$vencimiento=(strtotime($row->fecha_pago)-strtotime($row->fecha))/ (24 * 3600);

			echo "<tr background=\"$img_row\" align=\"center\"><td>$row->pr_facturas_id</td><td>$row->fecha</td><td>$row->folio_factura</td><td>$vencimiento</td><td>$row->espacio_fisico</td><td>$row->proveedor</td><td>".number_format($row->importe_factura,2,".",",")."</td><td>$row->estatus</td><td><a href=\"".base_url()."index.php/compras/compras_reportes/rep_pr_factura_pdf/$row->pr_facturas_id\" target=\"_blank\" class='modal_pdf1'>$photo</a> $auth $pass</td></tr>";
