...
<table class="listado" border="0" width="800">
	...
	if($polizas_e!=false){
		foreach($polizas_e->all as $row) {
			...
			$regenerar="<a href=\"".base_url()."index.php/".$ruta."/trans/regenerar_poliza_egreso/".$row->id." \" onclick=\"return confirm('¿Esta seguro que deseas regenerar la Póliza de Egreso?');\">$reload</a>";

			if($row->haber!=$row->debe)
				$cuadrar1="<a href=\"".base_url()."index.php/".$ruta."/trans/cuadrar_poliza_ingreso/".$row->id." \" onclick=\"return confirm('¿Esta seguro que deseas cuadrar y cerrar la Póliza de Egreso?');\">$cuadrar</a>";
			else
				$cuadrar1="";


			echo "<tr background=\"$img_row\"><td align=\"center\">$row->id</td><td align=\"center\">".fecha_imp($row->fecha)."</td><td>$row->espacio</td><td align=\"right\">$ ".number_format($row->debe, 2, ".", ",")."</td><td align=\"right\">$ ".number_format($row->haber, 2, ".", ",")."</td><td align=\"center\">$estatus</td><td><a href=\"".base_url()."index.php/contabilidad/contabilidad_reportes/poliza_egreso_pdf/".$row->id."\" target=\"_blank\" >$photo</a> $cuadrar1</td></tr>";
