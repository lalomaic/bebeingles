...
$select_beneficiarios[0] = "";
foreach ($beneficiarios as $beneficiario)
    $select_beneficiarios[$beneficiario->id] = $beneficiario->nombre;
$select_bancos[0] = "";
foreach ($bancos as $banco)
    $select_bancos[$banco->banco] = $banco->tag;
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#beneficiario_id').select_autocomplete({
            result: "setChange();"
        });
        $("#banco").select_autocomplete({
            result: "update_consecutivo();setChange();"
        });
        $('#fecha').change(function() {
            arr_old = fecha_old.split(' ');
            arr_new = $('#fecha').val().split(' ');
            if(guardada && (arr_old[1] != arr_new[1] || arr_old[2] != arr_new[2]))
                if(!confirm('Ha cambiado el mes de una poliza ya guardada, esto hara que se cancele el consecutivo del mes anterior.\n\
De cancelar si cambio el mes por error'))
                    $('#fecha').val(fecha_old);
            fecha_old = $('#fecha').val();
            update_consecutivo();
            setChange();
        ...
        $('#formPoliza').ajaxForm({
            ...
            error : function() {
                $("#validation_result").html("Error al guardar la poliza, por favor intente de nuevo");
                $("#saving_poliza").hide();
        ...
        $('.detalleCta:first').select_autocomplete({widthField:50, width:400});
    ...
    function deleteDetalle(id) {
        if(!confirm("Estas seguro que deseas eliminar el detalle " + id + " de la poliza?"))
            return;
    ...
    function update_consecutivo() {
        ...
        if(fecha_dep == $("#fecha").val() && banco == $("#banco").val()) {
            $('#consecutivo').val(consecutivo);
            return;
        }
        $.post(
        '<?php echo site_url("/ajax_pet/get_consecutivo_egreso") ?>',
        {mes:fecha[1],anio:fecha[2].substring(2,4), banco:$("#banco").val()},
        function(data){
    ...
    function salida(link) {
        if(!saved && !confirm("La cabezera est� penditente de guardar, a�n as� deseas salir?"))
            return;
        for(var i = 1; i < detalles.length; i++) {
            if(!detalles[i] && !confirm("Existen detalles penditentes de guardar, a�n as� deseas salir?"))
                return;
        }
    ...
    function crearRow() {
        ...
        select.select_autocomplete({widthField:50, width:400});
        td = td.next();
...
<table class="form_table">
    <tr>
        ...
        <td>
            ...
            <?php echo form_dropdown("beneficiario_id", $select_beneficiarios, 0, "id='beneficiario_id'") ?>
        </td>
    </tr><tr class="row">
        <td>
            <label for="concepto">Banco:</label>
        </td><td>
            <?php echo form_dropdown("banco", $select_bancos, 0, "id='banco'") ?>
        </td>
    </tr><tr>
        <td colspan="2">
...
<table class="row_detail" style="margin-top: 10px;" id="polizaDetalles">
    <tr>
        ...
        <td>
            ...
            <img id="imgWait1" class="img" src="<?php echo base_url() ?>images/waiting.gif" width="16" heigth="16" title="Realizando operaci�n"/>
    ...
    <tr>
        <th class="detalle_pie" colspan="6">
            ...
            <button type="button" onclick="salida('contabilidad/poliza_c/formulario/list_polizas_egresos');">Ir al listado</button>
            <button type="button" onclick="salida('contabilidad/poliza_c/formulario/alta_manual_poliza_egreso');">Generar otra poliza</button>
    </tr>
</table>