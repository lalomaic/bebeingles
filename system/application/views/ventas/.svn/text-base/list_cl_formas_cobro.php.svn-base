<?php 
echo "<h2>$title</h2>";
echo "<div align=\"left\"><a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/alta_cl_forma_cobro\"><img src=\"".base_url()."images/adduser.png\" width=\"50px\" title=\"Nueva Forma de Cobro\"></a></div>";
...
<table class="listado" border="0">
	...
	foreach($formas_cobros->all as $row) {
		...
		echo "<tr background=\"$img_row\"><td>$row->id</td><td>$row->tag</td><td><a href=\"".base_url()."index.php/".$ruta."/".$controller."/".$funcion."/".$subfuncion."/editar_cl_forma_cobro/".$row->id."\">$edit</a>$delete<a href=\"".base_url()."index.php/welcome/sespolice/quejas_pdf/".$row->id."\" target=\"_blank\">$photo</a></td></tr>";
