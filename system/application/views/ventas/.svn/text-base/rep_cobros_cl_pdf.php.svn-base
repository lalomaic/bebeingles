...
{
	if(count($cliente['cobros'])>0)
	{
		$pdf->SetFont('Times','B',8);
		$pdf->Cell(0,5,$cliente['nombre']);
		$pdf->ln();
		$pdf->SetAligns($cabs);
		$pdf->Row(array('Id','Fecha','Cuenta Origen','Cuenta Destino','Numero Referencia','Monto Pagado','Folio Factura','Usuario'));
		$pdf->SetFont('Times','',8);
		$pdf->SetAligns($filas);
		$total=(float)0;
		foreach($cliente['cobros'] as &$cobro)
		{
			...
			$pdf->Row(array($cobro['id'], $cobro['fecha'], $origen, $destino, $cobro['numero_referencia'], '$ '.number_format($cobro['monto_pagado'],2), $cobro['folio_factura'], $cobro['nombre']));
		}
