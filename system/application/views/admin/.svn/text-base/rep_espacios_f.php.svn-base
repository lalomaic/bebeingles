...
$empresas="<select name='empresas'>".$select1."</select>";//print_r(array_values($select1));//Construir Tipos de Espacios Fisicos
$select2=$default;
foreach($tipos_espacios->all as $row){
	$y=$row->id;
	$select2.="<option value='$y'>$row->tag</option>";
}
$tipos_espacios="<select name='tipos_espacios'>".$select2."</select>";//Titulo
echo "<h2>$title</h2>";
//Select para ordenar la consulta
$options="<option value='0'>Elija</option><option value='1'>Empresa</option><option value='2'>Tipo de local</option><option value='3'>Nombre de local</option>";
...
echo form_open($ruta."/".$ruta."_reportes/rep_espacios_f_pdf/", $atrib) . "\n";
		...
		<td width="30%" valign="top">
			<table border="1">
				...
				<tr>
					...
					<td colspan="3"><?php echo $empresas;?>
					</td>
				</tr>
				<tr>
					<td colspan="0">Tipo de Local:</td>
					<td colspan="3"><?php echo $tipos_espacios;?>
					</td>
				</tr>
				<tr>
