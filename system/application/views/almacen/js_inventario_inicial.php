...
// 		alert("Guardando .."+$('#precio_u'+linea).val());
		...
		if(producto>0 && producto_num>0 && cantidad_js>0 && f_id>0){
			$.post("<? echo base_url();?>index.php/almacen/trans/editar_pr_factura_inventario", { 
...
function enviar_inventario(linea,linea2){
		//Obtener campos clave producto_id y cantidad
...
// 		alert("Guardando .."+$('#precio_u'+linea).val());
		pr_fid=document.form1.id.value;
		if(f_id>0){
			$.post("<? echo base_url();?>index.php/almacen/trans/editar_inventario_inicial", { 
				cproductos_id: producto,
				cproducto_numero_id: producto_num,
				costo_unitario: precio_u,
				tasa_impuesto: $('#iva'+linea).val(),
...
function verificar(){
  var id=$('#f_id').val();
  location.href="<? echo base_url();?>index.php/<? echo $ruta; ?>/trans/verificar_entrada_inventario/"+id;
}
