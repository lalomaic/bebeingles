...
<table class="listado" border="0" width="980">
	<tr>
		<th>Id Factura</th>
		<th>Id Pedido</th>
		<th>Fecha y Hora</th>
	...
	if($salidas!=false){
		foreach($salidas->all as $row) {
			...
			echo "<tr background=\"$img_row\" align=\"center\"><td>$row->cl_facturas_id</td><td>$row->cl_pedido_id</td><td>$row->fecha</td><td>$row->espacio_fisico</td><td>$row->cliente</td><td>$row->importe_factura</td><td>$row->tipof</td><td>$row->estatus</td><td><a href=\"".base_url()."index.php/ventas/ventas_reportes/rep_cl_factura_pdf/$row->cl_facturas_id\" target=\"_blank\" class='modal_pdf1'>$photo</a> $delete</td></tr>";
		}
	}
	echo "</table></center>";
