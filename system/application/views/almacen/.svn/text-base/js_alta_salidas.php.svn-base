...
function get_ext(valor, linea, cantidad) {
  if(valor > 0 && cantidad>0) { 
    ...
    function(data){
      e=data;
      if(e<parseFloat(cantidad)){
      txt="<p id='mensaje'>Disponible: "+data+"</p>";
      $('#msg'+linea).html(txt);
      $('#cantidad'+linea).val('0')
...
function send_detalle(){
      $('#boton_p').hide();
      alert("La salida por facturaciÃ³n se esta procesando... Continuar");
...
function verificar(){
  ...
  if(id>0){
  location.href="<? echo base_url();?>index.php/<? echo $ruta; ?>/trans/verificar_salida/"+id;
  } else {
    location.href="<? echo base_url();?>index.php/<? echo $ruta; ?>/almacen_c/list_salidas";
  }
}

  function mostrar_subform(){
  for(var y=0;y< <? echo $rows; ?>;y++){
    eval("document.fdetail"+y+".cl_facturas_id"+y+".value=document.form1.id.value");
   var id=parseFloat(eval("document.fdetail"+y+".cl_facturas_id"+y+".value"));
  ...
  function act_parent_id(obj, line){
    ...
    eval(obj+".cl_facturas_id"+line+".value=document.form1.id.value");
  }
